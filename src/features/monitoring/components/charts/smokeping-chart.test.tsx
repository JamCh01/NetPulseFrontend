...
import { SmokePingChart } from './smokeping-chart'
...
vi.mock('@/components/charts/lazy-echarts', () => ({
  LazyECharts: ({ option }: { option: EChartsOption }) => {
    ...
    return <div data-testid="smokeping-chart" />
  },
}))
...
describe('SmokePingChart', () => {
  it('keeps the last rendered data while a refresh temporarily has no incoming data', () => {
    capturedOptions.length = 0

    const { rerender } = render(
      <SmokePingChart data={points} protocol="icmp" />,
    )

    expect(screen.getByTestId('smokeping-chart')).toBeInTheDocument()
    expect(capturedOptions.at(-1)?.title).toBeUndefined()

    rerender(
      <SmokePingChart data={[]} protocol="icmp" isUpdating />,
    )
